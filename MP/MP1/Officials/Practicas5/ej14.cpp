...
void ordena_vector(int v[], int n){
			int aux;
			for(int i=0; i<n; i++){			//Repite la lectura del vector.
						for(int j=0; j<n-1; j++){			//Ciclo de comprobaciones del orden, (n-1 comprobaciones).
									if(v[j]>v[j+1]){			//Si el siguiente es menor que el anterior...
												aux=v[j];
												v[j]=v[j+1];
												v[j+1]=aux;			//Intercambia valores.
									}
						}
			}
}
