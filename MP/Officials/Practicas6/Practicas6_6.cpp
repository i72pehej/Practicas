...
void max_row(float m[4][4], float max[4]){
			for (int i=0; i<4; i++){
						for(int j=0; j<4; j++){
									if(j==0){max[i]=m[i][j];}			//Guardamos en el vector los valores iniciales de las filas.
									if(max[i]<m[i][j]){max[i]=m[i][j];}			//Si los valores del vector son menores que los de la matriz, 																															guardamos el mayor valor.
						}
			}
}
