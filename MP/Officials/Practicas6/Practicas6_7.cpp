...
void min_col(float m[4][4], float min[4]){
		for (int i=0; i<4; i++){
				for(int j=0; j<4; j++){
						if(i==0){min[j]=m[i][j];}			//Guardamos en el vector los valores iniciales de las columnas.
						if(min[j]>m[i][j]){min[j]=m[i][j];}			//Si los valores del vector son mayores que los de la matriz, 																																guardamos el menor valor.
				}
		}
}
