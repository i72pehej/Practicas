//Ejercicio3.3 -> Hay un hilo productor y C hilos consumidores. Cada número generado por el productor ha de ser consumido por todos los consumidores.
...
int main(){
  pthread_t hilo_P;
  pthread_t hilo_C[N_Cons];
  int status, i, idProductor = 1, v_idConsumidor[N_Cons];
  ...
  for (size_t c = 2, i = 0; c < (1 + N_Cons), i < N_Cons; c++, i++) {
    v_idConsumidor[i] = c;
...
void *consumidor(void *c){
  ...
  extern int buffer[N];
  int *id, i;
  int nConsumiciones = ITER / N_Cons;
